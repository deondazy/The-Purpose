...
if ($user->count(['username' => $author])['count'] == 0) {
    $flash->set('error', 'Invalid Action');
    Utility::redirect($config->site->url . '/blog/');
}

// Get Author ID
$authorId = $user->getAll('id', ['where' => ['username' => $author]])[0]['id'];
...
.page-link {
    color: var(--thm-primary);
        ...
        <section class="page-header">
            ...
            <!-- /.page-header__bg -->
            <div class="container">
                <h2>Author: <?= $user->get('display_name', $authorId)['display_name'] ?></h2>
            </div>
        </section>
        <!--Page Header End-->
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <ul class="pagination pagination-flat justify-content-center mt-4">
                                    <?php 
                                    $prev_link = ($currentPage > 1) ? $config->site->url . "/blog/author/{$author}/page/" . ($currentPage - 1) : null;
                                    $next_link = ($currentPage < $totalPages) ? $config->site->url . "/blog/author/{$author}/page/" . ($currentPage + 1) : null;
                                    ?>
                                    ...
                                    <?php for ($i = 1; $i <= $totalPages; $i++) :?>
                                        <li class="page-item <?= ($currentPage == $i) ? 'active' : '' ?>">
                                            <a href="<?= $config->site->url . '/blog/author/' . $author . '/page/' . $i . '/' ?>" class="page-link rounded"><?= $i ?></a>
                                        </li>
                                    <?php endfor; ?>
