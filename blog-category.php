...
if ($category->count(['slug' => $categorySlug])['count'] == 0) {
    $flash->set('error', 'Invalid Action');
    Utility::redirect($config->site->url . '/blog/');
}

// Get Author ID
$categoryId = $category->getAll('id', ['where' => ['slug' => $categorySlug]])[0]['id'];
...
.page-link {
    color: var(--thm-primary);
        ...
        <section class="page-header">
            ...
            <!-- /.page-header__bg -->
            <div class="container">
                <h2>Category: <?= $category->get('name', $categoryId)['name'] ?></h2>
            </div>
        </section>
        <!--Page Header End-->
            ...
            <div class="container">
                <div class="row">
                    ...
                    $totalPosts = Select::new($connection)
                        ->columns("COUNT(DISTINCT pc.post_id) as count")
                        ->from('post_categories pc')
                        ->join('LEFT', 'posts p', 'pc.post_id = p.id')
                        ->whereEquals(['p.status' => 'PUBLISH'])
                        ->whereEquals(['pc.category_id' => $categoryId])
                        ->fetchOne()['count'];
